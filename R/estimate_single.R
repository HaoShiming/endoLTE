...
#' @param time Numeric vector of time index
...
#' The method involves:
#' 1. Bernstein polynomial expansion of treatment and outcome processes
...
#' z <- rnorm(n)
#' tr <- 10 + 0.5 * sin(2 * pi * time / n)
#' y <- 5 + 2 * w + 0.5 * z + tr * D + rnorm(n)
#' 
#' # Estimate treatment effect at first post-treatment period
#' effect <- estimate_single_series(y = y, D = D, z = z, w = w, time = time, t_post = 1)
#' print(effect)
#' 
#' @export
estimate_single_series <- function(y, D, z = NULL, w = NULL, time, t_post = 1, previous_estimates = NULL) {
  
  # Input validation
  if (missing(y) || missing(D) || missing(time)) {
    stop("y, D, and time must be specified")
  }
  
  if (length(y) != length(D) || length(y) != length(time)) {
    stop("y, D, and time must have the same length")
  }
