...
#' @param time_var Name of the user-defined variable (e.g., time) used as psi (character)
...
#' 
#' # Estimate treatment effects (using "time" as psi)
#' result <- estimate_panel_lte(
#'   data = panel_data,
#'   unit_id = "unit_id",
#'   time_var = "time",    # this variable will be used as psi (user defined common proximal variable)
#'   outcome = "y",
#'   treatment = "D",
#'   covariates = "z",
#'   confounders = "w",
...
estimate_panel_lte <- function(data, unit_id, time_var, outcome, treatment,
  ...
  for (i in seq_along(treated_units)) {
    ...
    psi <- unit_data[[time_var]]  # rename time_var to psi for clarity
    ...
    # Estimate treatment effects for this unit (pass psi instead of time)
    unit_result <- estimate_unit_effects(
      y = y, D = D, z = z, w = w, psi = psi, 
...
estimate_unit_effects <- function(y, D, z = NULL, w = NULL, psi, true_effects = NULL,
                                  n_post_periods = 10, bootstrap_iter = 100) {
  ...
  for (t in 1:max_post) {
    ...
    # Point estimate (pass psi instead of time)
    ...
    if (!is.na(estimates[t])) {
      bootstrap_result <- bootstrap_unit(
        y = y, D = D, z = z_vec, w = w_vec, psi = psi,
...
bootstrap_unit <- function(y, D, z = NULL, w = NULL, psi, t_post, 
  ...
  for (b in 1:B) {
    tryCatch({
      ...
      if (is.null(z) && is.null(w)) {
        model <- stats::lm(y ~ D + psi)
      } else if (is.null(w)) {
        model <- stats::lm(y ~ D + psi + z)
      } else if (is.null(z)) {
        model <- stats::lm(y ~ D + psi + w)
      } else {
        model <- stats::lm(y ~ D + psi + z + w)
      }
      
      # Generate new outcome with same residuals
      y_boot <- stats::fitted(model) + sample(stats::resid(model), replace = TRUE)
      
      # Re-estimate on bootstrap sample (pass psi instead of time)
      bootstrap_estimates[b] <- estimate_single_series(
        y = y_boot, D = D, z = z, w = w, psi = psi,
...
# Helper function: Calculate pooled effects (unchanged)
...
# Helper function: Calculate performance metrics (unchanged)
